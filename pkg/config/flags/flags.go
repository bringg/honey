...
func setDefaultFromEnv(flags *pflag.FlagSet, name string) {
	key := place.OptionToEnv(name)
	newValue, found := os.LookupEnv(key)
	if found {
		flag := flags.Lookup(name)
		if flag == nil {
			logrus.Fatalf("Couldn't find flag --%q", name)
		}

		err := flag.Value.Set(newValue)
		if err != nil {
			logrus.Fatalf("Invalid value for environment variable %q when setting default for --%s: %v", key, name, err)
		}

		logrus.Debugf("Set default for --%q from %q to %q (%v)", name, key, newValue, flag.Value)
